export const about = [
    {
        type: "Education",
        title: "Junior High School",
        place: "Valley High Academy, Inc.",
        period: "2015-2019",
        description: "Graduated with honors and best in Mathematics award.",
        tags: []
    },
    {
        type: "Education",
        title: "Senior High School",
        place: "Our Lady of Fatima University",
        period: "2019-2021",
        description: "Graduated in Science, Technology, Engineering, and Mathematics (STEM) strand with honors.",
        tags: []
    },
    {
        type: "Education",
        title: "College",
        place: "Polytechnic University of the Philippines",
        period: "2021-2025",
        description: "Graduated with the degree of Bachelor of Science in Computer Engineering with a specialization in Software Development.",
        tags: []
    },
    {
        type: "Experience",
        title: "Web Developer Intern",
        place: "Marvill Web Development Philippines",
        period: "2023",
        description: "Collaborated with designers and team members to enhance the UI/UX of web projects.",
        tags: ["Laravel", "PHP", "MySQL", "WordPress", "Adobe Illustrator"]
    },
    {
        type: "Experience",
        title: "Front-End Developer Intern",
        place: "LexMeet, Inc.",
        period: "2024",
        description: "Contributed to creating responsive and modern website designs tailored to client needs.",
        tags: ["React", "Bootstrap", "JavaScript", "Figma", "GitLab"]
    },
    {
        type: "Experience",
        title: "System Developer",
        place: "Polytechnic University of the Philippines",
        period: "2025",
        description: "Designed and developed a thesis project which is a web-based laboratory equipment management system to monitor equipment usage, prevent loss, and improve resource allocation.",
        tags: ["React", "Firebase", "Bootstrap", "JavaScript", "SCSS", "Figma", "GitLab"]
    },
]

export const projects = [
    {
        title: "mclrrn",
        collaboration: "Personal",
        description: "A personal website built using Next.js and Tailwind CSS to showcase my skills, projects, and experience.",
        img: "/images/mclrrn-shot.png",
        stack: ["Next.js", "Tailwind CSS", "TypeScript"],
        website: "https://mclrrn.vercel.app",
        isInGitHub: true,
        github: "https://github.com/micaporras/personal_website",
        isDeployed: true,
    },
    {
        title: "LEDice",
        collaboration: "Collaborative",
        description: "A web-based dice roller application built with React and Firebase, allowing users to roll virtual dice for the color game or mix colors for color mixer.",
        img: "/images/LEDice.png",
        stack: ["React", "Firebase", "Bootstrap", "SCSS", "Arduino IDE"],
        website: "https://ledice-66ce7.web.app/",
        isInGitHub: true,
        github: "https://github.com/micaporras/ledice",
        isDeployed: true,
    },
    {
        title: "CpE LEMS",
        collaboration: "Collaborative",
        description: "A web-based laboratory equipment management system designed to monitor equipment usage, prevent loss, and improve resource allocation for the Computer Engineering department at the Polytechnic University of the Philippines.",
        img: "/images/CpELEMS.png",
        stack: ["React", "Firebase", "Bootstrap", "SCSS"],
        website: "https://cpe-lems.web.app/user/form",
        isInGitHub: false,
        github: "https://gitlab.com/porrasmica15/lems",
        isDeployed: true,
    },
    {
        title: "Iris",
        collaboration: "Collaborative",
        description: "A web-based application  that helps one to be reminded of certain tasks that need to be done. It has a main feature of a To-Do List but offers much more features which ensures the users that not a single task will be neglected.",
        img: "/images/iris-shot.png",
        stack: ["HTML", "CSS", "Bootstrap", "PHP", "Laravel", "MySQL"],
        website: "",
        isInGitHub: true,
        github: "https://github.com/micaporras/iris",
        isDeployed: false,
    },
    {
        title: "Personal Website Sample",
        collaboration: "Collaborative",
        description: "A simple personal website built using HTML, CSS, JavaScript. It serves as a portfolio to showcase projects and skills.",
        img: "/images/personal-website.png",
        stack: ["HTML", "CSS", "JavaScript"],
        website: "https://micaporras.github.io/Responsive-Website/",
        isInGitHub: true,
        github: "https://github.com/micaporras/Responsive-Website",
        isDeployed: true,
    },
    {
        title: "Registration Form Sample",
        collaboration: "Collaborative",
        description: "A simple registration form built using HTML, CSS, JavaScript, PHP, and MySQL. It allows users to register by providing their information, which is then stored in a MySQL database.",
        img: "/images/regform.png",
        stack: ["HTML", "CSS", "JavaScript", "PHP", "MySQL"],
        website: "",
        isInGitHub: true,
        github: "https://github.com/micaporras/regform",
        isDeployed: false,
    },
]

export const frontendAndBackend = [
    {
        name: "HTML5",
        alt: "HTML5 Logo",
        icon: "/icons/html5.svg"
    },
    {
        name: "CSS3",
        alt: "CSS3 Logo",
        icon: "/icons/css.svg"
    },
    {
        name: "Javascript",
        alt: "Javascript Logo",
        icon: "/icons/javascript.svg"
    },
    {
        name: "ReactJS",
        alt: "ReactJS Logo",
        icon: "/icons/react.svg"
    },
    {
        name: "Bootstrap",
        alt: "Bootstrap Logo",
        icon: "/icons/bootstrap.svg"
    },
    {
        name: "SCSS",
        alt: "SCSS Logo",
        icon: "/icons/sass.svg"
    },
    {
        name: "Typescript",
        alt: "Typescript Logo",
        icon: "/icons/typescript.svg"
    },
    {
        name: "NextJS",
        alt: "NextJS Logo",
        icon: "/icons/nextjs.svg"
    },
    {
        name: "TailwindCSS",
        alt: "TailwindCSS Logo",
        icon: "/icons/tailwindcss.svg"
    },
    {
        name: "MySQL",
        alt: "MySQL Logo",
        icon: "/icons/mysql.svg"
    },
    {
        name: "PHP",
        alt: "PHP Logo",
        icon: "/icons/php.svg"
    },
    {
        name: "Laravel",
        alt: "Laravel Logo",
        icon: "/icons/laravel.svg"
    },
    {
        name: "Python",
        alt: "Python Logo",
        icon: "/icons/python.svg"
    }
]

export const tools = [
    {
        name: "Git",
        alt: "Git Logo",
        icon: "/icons/git.svg"
    },
    {
        name: "GitHub",
        alt: "GitHub Logo",
        icon: "/icons/github.svg"
    },
    {
        name: "GitLab",
        alt: "GitLab Logo",
        icon: "/icons/gitlab.svg"
    },
    {
        name: "VS Code",
        alt: "Visual Studio Code logo",
        icon: "/icons/vscode.svg"
    },
    {
        name: "Vite",
        alt: "Vite Logo",
        icon: "/icons/vite.svg"
    },
    {
        name: "Firebase",
        alt: "Firebase Logo",
        icon: "/icons/firebase.svg"
    },
    {
        name: "Figma",
        alt: "Figma Logo",
        icon: "/icons/figma.svg"
    },
    {
        name: "GCP",
        alt: "Google Cloud Platform",
        icon: "/icons/googlecloud.svg"
    },
    {
        name: "Word",
        alt: "Microsoft Word Logo",
        icon: "/icons/word.svg"
    },
    {
        name: "Excel",
        alt: "Microsoft Excel Logo",
        icon: "/icons/excel.svg"
    },
    {
        name: "PowerPoint",
        alt: "Microsoft Powerpoint logo",
        icon: "/icons/powerpoint.svg"
    },
    {
        name: "Illustrator",
        alt: "Adobe Illustrator Logo",
        icon: "/icons/illustrator.svg"
    },
    {
        name: "Photoshop",
        alt: "Adobe Photoshop Logo",
        icon: "/icons/photoshop.svg"
    },
    {
        name: "Jira",
        alt: "Jira Logo",
        icon: "/icons/jira.svg"
    },
    {
        name: "Arduino IDE",
        alt: "Arduino IDE Logo",
        icon: "/icons/arduino.svg"
    },
    {
        name: "WordPress",
        alt: "WordPress Logo",
        icon: "/icons/wordpress.svg"
    },
]

export const devPractices = [
    {
        name: "OOP",
        alt: "Object Oriented Programming Logo",
        icon: "/images/oop.png"
    },
    {
        name: "Responsive Development",
        alt: "Responsive Development Logo",
        icon: "/images/responsive.png"
    },
    {
        name: "Version Control",
        alt: "Version Control Logo",
        icon: "/images/vc.png"
    },
    {
        name: "CI/CD Concepts",
        alt: "CI/CD Concepts Logo",
        icon: "/images/cicd.png"
    },
    {
        name: "Clean Code Principles",
        alt: "Clean Code Principles Logo",
        icon: "/images/clean-code.png"
    },
]

export const softSkills = [
    {
        name: "Effective Communication",
        alt: "Effective Communication Logo",
        icon: "/images/communicative.png"
    },
    {
        name: "Problem Solving",
        alt: "Problem Solving Logo",
        icon: "/images/problem-solving.png"
    },
    {
        name: "Collaboration & Teamwork",
        alt: "Collaboration & Teamwork Logo",
        icon: "/images/teamwork.png"
    },
    {
        name: "Detail-Oriented",
        alt: "Detail-Oriented Logo",
        icon: "/images/detail-oriented.png"
    },
    {
        name: "Adaptable",
        alt: "Adaptable Logo",
        icon: "/images/adaptable.png"
    },
]